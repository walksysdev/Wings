...
os_check(){
    ...
    if [ "$lsb_dist" =  "ubuntu" ]; then
        # تم التعديل هنا للسماح بـ Ubuntu 24.04
        if  [ "$dist_version" != "20.04" ] && [ "$dist_version" != "22.04" ] && [ "$dist_version" != "24.04" ]; then
            output "Unsupported Ubuntu version. Only Ubuntu 20.04, 22.04, and 24.04 are supported."
            exit 2
        fi
    elif [ "$lsb_dist" = "debian" ]; then
        if [ "$dist_version" != "11" ] && [ "$dist_version" != "12" ]; then
            output "Unsupported Debian version. Only Debian 11 and 12 are supported."
            exit 2
        fi
    elif [ "$lsb_dist" = "fedora" ]; then
        if [ "$dist_version" != "35" ] && [ "$dist_version" != "39" ] && [ "$dist_version" != "40" ]; then
            output "Unsupported Fedora version."
            exit 2
        fi
    elif [ "$lsb_dist" = "centos" ]; then
        if [ "$dist_version" != "8" ] && [ "$dist_version" != "9" ]; then
            output "Unsupported CentOS version."
            exit 2
        fi
    elif [ "$lsb_dist" = "rhel" ]; then
        if  [ $dist_version != "8" ] && [ $dist_version != "9" ]; then
            output "Unsupported RHEL version."
            exit 2
        fi
    elif [ "$lsb_dist" = "rocky" ]; then
        if [ "$dist_version" != "8" ] && [ "$dist_version" != "9" ]; then
            output "Unsupported Rocky Linux version."
            exit 2
        fi
    elif [ "$lsb_dist" = "almalinux" ]; then
        if [ "$dist_version" != "8" ] && [ "$dist_version" != "9" ]; then
            output "Unsupported AlmaLinux version."
            exit 2
        fi
